...
for (let i = 0; i < signatureCanvases.length; i++) {
  ...
  signaturePad.addEventListener("beginStroke", function () {
    document
      .querySelector(
        ".js-sig-reset[for=" + signatureCanvas.getAttribute("for") + "]"
      )
  ...
  signaturePad.addEventListener("endStroke", function () {
    document.querySelector(
      ".js-sig[name=" + signatureCanvas.getAttribute("for") + "]"
    ).value = signaturePad.toDataURL();
    if (
      statusArray.every((val) => val == false) &&
      statusArray.length == signatureCanvases.length
    ) {
